...
def insta_open(file):
    ...
    df['post_timestamp'] = pd.to_datetime(post_timestamps, '%y-%m-%d %H:%M:%S')
    ...
    comments_df['comment_timestamp'] = pd.to_datetime(comments_timestamps, format='%y-%m-%d %H:%M:%S')
    ...
    # Add some features
    IG_full['isReply'] = np.where(IG_full['post_user_id'] == IG_full['comment_user_id'], 'Yes', 'No')

    IG_full = IG_full.reset_index(drop=True)
    IG_full['comment_emojis'] = IG_full['comment_text'].apply(lambda x: extract_emojis(x))
    IG_full['comment_emojis_decoded'] = IG_full['comment_emojis'].apply(lambda x: emoji.demojize(x))
